...
async def approve_task(assigned_id: int,db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    #     logger.error(f"approved_by is missing after approval for assignment {payload.assignment_id}.")
    ...
    #     logger.error(f"assigned_on is missing for assignment {payload.assignment_id}.")
